...
ensure_env() {
  ...
  # --- TTS Engine ---
  echo "Which TTS engine would you like to use?"
  echo "  1) kokoro      — fast, GPU-accelerated (default)"
  echo "  2) pocket_tts  — lightweight, 8 voices, CPU-friendly"
  printf "TTS engine [kokoro]: "
  read -r tts_input
  case "$tts_input" in
    2|pocket_tts|pocket) tts_engine="pocket_tts" ;;
    *)                   tts_engine="kokoro" ;;
  esac
  echo ""

  # --- Voice selection ---
  local kokoro_voice="af_heart"
  local pocket_voice="alba"

  if [[ "$tts_engine" == "kokoro" ]]; then
    echo "Choose a Kokoro voice (af_heart, am_adam, bf_emma, bm_george, ...)."
    echo "See README for the full list of 28 voices."
    printf "Kokoro voice [af_heart]: "
    read -r voice_input
    [[ -n "$voice_input" ]] && kokoro_voice="$voice_input"
  else
    echo "Choose a Pocket-TTS voice:"
    echo "  alba, fantine, cosette, eponine, azelma, marius, javert, jean"
    printf "Pocket-TTS voice [alba]: "
    read -r voice_input
    [[ -n "$voice_input" ]] && pocket_voice="$voice_input"
  fi
  echo ""

  # --- LLM Backend ---
  echo "Which LLM backend would you like to use?"
  echo "  1) ollama     — local Ollama server (default)"
  echo "  2) koboldcpp  — KoboldCpp API"
  printf "LLM backend [ollama]: "
  read -r llm_input
  case "$llm_input" in
    2|koboldcpp|kobold) llm_backend="koboldcpp" ;;
    *)                  llm_backend="ollama" ;;
  esac
  echo ""

  # --- Koboldcpp URL ---
  local kobold_url="http://localhost:5001"
  if [[ "$llm_backend" == "koboldcpp" ]]; then
    printf "Koboldcpp URL [http://localhost:5001]: "
    read -r url_input
    [[ -n "$url_input" ]] && kobold_url="$url_input"
    echo ""
  fi

  # --- LLM Model ---
  echo "LLM model name (leave blank for auto-detection):"
  printf "LLM model []: "
  read -r llm_model
  echo ""
  ...
  # --- Write .env ---
  {
    ...
    echo ""
    echo "# OVA configuration"
    echo "OVA_TTS_ENGINE=${tts_engine}"
    [[ "$tts_engine" == "kokoro" ]]    && echo "OVA_KOKORO_VOICE=${kokoro_voice}"
    [[ "$tts_engine" == "pocket_tts" ]] && echo "OVA_POCKET_TTS_VOICE=${pocket_voice}"
    echo "OVA_LLM_BACKEND=${llm_backend}"
    [[ -n "$llm_model" ]]              && echo "OVA_LLM_MODEL=${llm_model}"
    [[ "$llm_backend" == "koboldcpp" ]] && echo "OVA_KOBOLDCPP_URL=${kobold_url}"
  } > "$env_file"
  ...
  if [[ "$tts_engine" == "kokoro" ]]; then
    ...
    echo "  Pocket voice:  ${pocket_voice}"
  fi
  echo "  LLM backend:  ${llm_backend}"
  [[ -n "$llm_model" ]] && echo "  LLM model:    ${llm_model}"
  [[ "$llm_backend" == "koboldcpp" ]] && echo "  Koboldcpp URL: ${kobold_url}"
  echo ""
  echo "  You can edit .env any time to change these."
  echo "============================================"
  echo ""
}
